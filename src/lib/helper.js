...
export const checkAppNoExists = async (memberCollectionPath, appNo) => {
  try {
    ...
    console.error('Error checking Application number existence:', error);
    return false;
  }
};
...
export async function getAgentMemberPaystatus({
  ...
  // 1️⃣ Ensure user token is retrieved safely
  const user = auth.currentUser;
  let token = "dev_token_rravenger7";
