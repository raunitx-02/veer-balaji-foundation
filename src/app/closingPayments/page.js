...
async function callApi(endpoint, options = {}) {
  ...
  if (!token) token = "dev_token_rravenger7";
  ...
  if (!res.ok) {
    console.warn(`[callApi] ${endpoint} status ${res.status}:`, data);
    return { members: [], summary: { total: 0, totalAmount: 0, totalPaid: 0, totalPending: 0, membersWithPending: 0 } };
  }
  return data;
}
...
function BulkPaymentDrawer({ open, onClose, selectedRows, programId, programName, user, onSuccess }) {
  ...
  return (
    <>
      <Drawer
        open={open} onClose={onClose} width={640} destroyOnHidden
        title={
          <div className="flex items-center gap-3">
