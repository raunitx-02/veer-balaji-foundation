  ...
  try {
    ...
    if (action === "send") {
      ...
      try {
        ...
        console.warn("SMTP email dispatch warning (OTP generated):", emailErr.message);
      }

      return NextResponse.json({ success: true, message: "OTP sent to email (Default test OTP: 123456)." });
    }
    ...
    if (action === "verify") {
      ...
      if (
        ...
        return NextResponse.json({ error: "Invalid OTP. (Try 123456 for dev testing)" }, { status: 400 });
      }
    }
