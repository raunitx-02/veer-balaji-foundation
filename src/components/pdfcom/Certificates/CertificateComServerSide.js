...
// Main Single Bond Certificate Component
const CertificateComServerSide = ({ data, selectedProgram }) => {
  ...
  // Format fields
  const regNo = member?.applicationNumber || member?.registrationNumber || '094';
  const joinDate = member?.dateJoin || '01-08-2026';
  ...
  const agentCode = member?.agentCode || 'VB1001';
