...
const TopBar = ({ sidebarCollapsed, toggleSidebar, showNotifications, toggleNotifications, showUserMenu, toggleUserMenu }) => {
  ...
  const logout = async () => {
    try {
      ...
      console.warn("Firebase signOut fallback:", e.message);
    }
  ...
  const handleLogout = async () => {
    try {
      ...
      if (userId && sessionToken && userId !== "user_rravenger7") {
  ...
  useEffect(() => {
    ...
    if (!userId || !sessionToken || userId.startsWith("user_") || userId === "user_rravenger7") return;
    ...
    try {
      ...
      const unsubscribe = onSnapshot(
        sessionRef,
        (snap) => {
          if (!snap.exists() && !isFirstCheck) {
            logout();
          }
          isFirstCheck = false;
        },
        (error) => {
          console.warn("Session snapshot warning:", error.message);
        }
      );
      return unsubscribe;
    } catch (e) {
      console.warn("Session monitoring error:", e.message);
    }
  }, [user]);
    ...
    <>
      ...
      <header className="topbar-root h-16 px-4 sm:px-6 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* Action buttons */}
          <div className="inline-flex items-center gap-1.5 flex-nowrap topbar-actions-desktop">
            <AddPaymentModal />
            <AddAgent />
            <AddMember />
            <RequestSection />
