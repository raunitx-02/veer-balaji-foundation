...
export default function CustomDashboardLayout({ children }) {
    ...
    for (const key in clean) {
      if (clean[key] && typeof clean[key] === 'object') {
        if (typeof clean[key].toDate === 'function') {
          ...
          clean[key] = new Date(clean[key].seconds * 1000).toISOString();
        }
      }
    }
    return clean;
  ...
  const getAgentData = async () => {
    try {
      ...
      dispatch(setAgentList(snap.docs.map(d => ({ id: d.id, ...sanitizeData(d.data()) }))));
    ...
    try {
      ...
      if (user?.uid) {
        try {
          ...
          const snap = await getDocs(query(col, orderBy('createdAt', 'desc')));
          programs = snap.docs.map(d => ({ id: d.id, ...sanitizeData(d.data()) }));
        } catch (fetchErr) {
          console.warn("Firestore program fetch warning:", fetchErr.message);
        }
      }
      ...
      console.error(e);
      dispatch(setPrograms(defaultYojnas));
  ...
  }, [user?.uid, agentStatusChanged, memberStatusChanged]);
