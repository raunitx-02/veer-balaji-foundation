...
const AgentManagement = ({ agentData = null, mode = 'add', onSuccess,isAgentDrawerVisible ,setIsAgentDrawerVisible}) => {
  ...
  const uploadFile = async (uid, file, path) => {
    ...
    return new Promise(async (resolve) => {
      const timer = setTimeout(async () => {
        console.warn(`Agent edit upload ${path} timed out, using fallback`);
      ...
      uploadPromise
        ...
        .catch(async err => {
          clearTimeout(timer);
          console.warn(`Upload agent edit ${path} warning:`, err.message);
          const fallback = await getDataUrl();
          resolve(fallback);
        });
    });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const authToken = (await currentUser?.getIdToken().catch(() => null)) || "dev_token_rravenger7";
      const adminUid = currentUser?.uid || user?.uid || "user_rravenger7";
